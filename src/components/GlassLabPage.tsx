...
function Slider({ label, min, max, step, value, onChange }: {
  ...
  return (
    <label className="flex items-center gap-3 text-[13px] text-ink/70">
      ...
      />
      <span className="w-10 shrink-0 text-right tabular-nums text-ink/90">{value}</span>
    </label>
  )
}
  ...
  return (
    <div className="animate-fade-in flex flex-col gap-8 pb-48">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-instrument text-[22px] font-semibold text-ink/80">Glass lab</h1>
        <button
          onClick={() => { window.location.href = window.location.pathname }}
          className="flex items-center gap-1.5 rounded-full bg-ink/10 px-3.5 py-1.5 text-[13px] text-ink/80"
        >
      ...
      {glassMode === 'webgl' && (
        <div className="flex flex-col gap-2 rounded-2xl bg-ink/5 p-4">
          <h2 className="mb-1 text-[13px] uppercase tracking-wide text-ink/40">WebGL shader FX</h2>
      ...
      <div className="flex flex-col gap-2 rounded-2xl bg-ink/5 p-4">
        ...
        <button
          className="mt-1 self-start rounded-lg bg-ink/10 px-3 py-1 text-[13px] text-ink/80"
      ...
      <div className="flex flex-col gap-3">
        <h2 className="text-[13px] uppercase tracking-wide text-ink/40">Reference panes (production params)</h2>
        <GlassPane borderRadius={24} className="rounded-[24px] p-5">
          <span className="font-instrument text-[18px] font-semibold text-ink/90">Card pane</span>
        </GlassPane>
        <div className="flex items-center gap-3">
          <GlassPane borderRadius={20} className="rounded-[20px] px-4 py-2">
            <span className="text-[14px] text-ink/90">Pill</span>
          </GlassPane>
          <GlassPane borderRadius={12} className="rounded-[12px] px-2 py-1">
            <span className="text-[12px] text-ink/80">Tiny</span>
          </GlassPane>
        </div>

        {/* Button variants — the app's real GlassButton styles */}
        <div className="flex items-center gap-3">
          <GlassButton radius={16} pane="bg-accent/10" className="flex-1 border border-accent/20 py-3 font-instrument text-[15px] font-medium text-accent">
            Primary
          </GlassButton>
          ...
          <GlassButton radius={16} pane="bg-ink/5" className="flex-1 border border-ink/10 py-3 font-instrument text-[15px] font-medium text-ink/70">
            Neutral
          </GlassButton>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <GlassButton radius={32} pane="bg-accent/15" className="h-[60px] w-[60px] border border-accent/30 shadow-[0_0_24px_rgba(180,160,255,0.25)]">
            <span className="material-symbols-rounded text-[28px] text-accent">play_arrow</span>
          </GlassButton>
          <GlassButton radius={20} pane="bg-ink/5" className="h-[38px] w-[38px] border border-ink/10 text-ink/50">
      ...
      <div className="flex flex-col gap-5">
        <h2 className="text-[13px] uppercase tracking-wide text-ink/40">Mask shapes (live params)</h2>
        {SHAPES.map(s => (
          <div key={s.name} className="flex flex-col gap-1.5">
            <span className="text-[12px] text-ink/40">{s.name}</span>

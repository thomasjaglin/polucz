  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-[20px] border border-[#F8FAFC]/10 bg-[#F8FAFC]/[0.02] px-5 py-6 text-center">
        ...
        <div className="mt-2 font-instrument text-[14px] text-[#F8FAFC]/40">{timeStr}</div>
      </div>

      {/* Wrong answers */}
      {wrong.length > 0 && (
        <div className="flex flex-col gap-2">
          <p className="font-instrument text-[12px] uppercase tracking-wider text-[#F8FAFC]/35">Review</p>
          {wrong.map((a, i) => (
            <div key={i} className="rounded-[16px] border border-red-400/15 bg-red-400/[0.04] px-4 py-3">
              <p className="line-clamp-1 font-instrument text-[12px] text-[#F8FAFC]/35">{a.polish}</p>
              <div className="mt-1.5 flex items-center gap-2">
                <span className="font-instrument text-[15px] text-red-400/65 line-through">{a.given || '—'}</span>
                <span className="material-symbols-rounded text-[14px] text-[#F8FAFC]/25">arrow_forward</span>

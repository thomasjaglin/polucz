...
export default function DebugHud() {
  ...
  return (
    <>
      ...
      >
        {`mode: ${mode}  panes: ${panes}/48 (${sorted.length} real)  masks: ${masks}/2\n` +
          (glassDebug.lastError ? `ERROR @frame ${glassDebug.lastErrorFrame}: ${glassDebug.lastError.slice(0, 150)}\n` : '') +
          `\nALL panes: center vs 2px-from-edge (canvas only, no DOM tint):\n` +
          glassDebug.allPaneSamples.map((p, i) => `${i} (${p.w.toFixed(0)}x${p.h.toFixed(0)} @${p.x.toFixed(0)},${p.y.toFixed(0)}):\n  center=${p.center.slice(0,3).join(',')}  edge2px=${p.edge2px.slice(0,3).join(',')}`).join('\n')}
      </div>

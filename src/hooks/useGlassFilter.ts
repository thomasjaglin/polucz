...
// A live element whose inscribed ellipse occludes a pane's glass (the pane
// reads as sliding under that shape). Passed as a ref so the renderer reads its
...
export function useGlassFilter(borderRadius: number, rotation?: MotionValue<number>, clipEllipseRef?: ClipEllipseRef) {
  ...
  useEffect(() => {
    ...
    if (mode === 'webgl') {
      ...
      const getClipEllipse = clipEllipseRef
        ? () => {
            const ce = clipEllipseRef.current
            if (!ce) return null
            const r = ce.getBoundingClientRect()
            return { cx: r.left + r.width / 2, cy: r.top + r.height / 2, rx: r.width / 2, ry: r.height / 2 }
          }
        : undefined
      return registerPane({ el, borderRadius, getRotation, getClipEllipse })
    }
    if (mode !== 'svg') return
  ...
  }, [filterId, borderRadius, mode, rotation, clipEllipseRef])
